...
import java.util.*;
import com.ices.discrete_event_simulation.cyf.pathVariable;
...
//            completeXml2(poolNode);
    ...
    private static void getInteractionInformation() {
        ...
        for(int i=0;i<messageNodes.size();++i){
            ...
            String graphNodeIdReceive = messageNode.attributeValue("targetRef");
            String graphNodeIdSend = messageNode.attributeValue("sourceRef");
            allNodes.get(graphNodeIdReceive).setReceiveInteractionInformation(interactionName);
            allNodes.get(graphNodeIdSend).setSendInteractionInformation(interactionName);
        }
...
//            newFirstNode=poolNode.getPoolGraphNodes().get(0);
//        }
        ...
        for (GraphNode node:graphNodes){
            if(node.getNodeState()==3){
                ...
                content.append("<userTask id=\""+node.getGraphId()+"\" name=\""+node.getGraphName()+"\"><documentation id=\""+"s"+UUID.randomUUID().toString()+"\">"+node.getReceiveInteractionInformation()+","+node.getSendInteractionInformation()+"</documentation></userTask>\n");
                //节点后无内容加一个endEvent
    ...
    private static void completeXml2(PoolNode poolNode) {
        ...
        for(GraphNode graphNode:poolAllNodes){
            if(graphNode.getNodeState()==1){
                String randomEnd="s"+UUID.randomUUID().toString();
                content.append("<userTask id=\""+graphNode.getGraphId()+"\" name=\""+graphNode.getGraphName()+"\"><documentation id=\""+"s"+UUID.randomUUID().toString()+"\">"+graphNode.getReceiveInteractionInformation()+","+graphNode.getSendInteractionInformation()+"</documentation></userTask>\n");
        ...
        content.append("\t</process>\n" +
                "</definitions>\n");
        WriteIntoFile3(poolNode.getProcessId(),content.toString());
        WriteIntoFile4(poolNode.getProcessId(),content.toString());
    }
...
//            }
    ...
    private static void WriteIntoFile4(String processId,String xmlStr){
        String poolName = poolNameToFederateName.get(processId);
        File file = null;
        FileWriter fw =null;
        try{
